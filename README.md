...
SlideTwin (formerly PPT Visual Reconstructor) is a Codex plugin and Windows companion toolkit for creating a high-fidelity editable PowerPoint slide from only a reference image.
...
> 中文概述：使用者只需上传一张PPT参考图片，SlideTwin 自动生成语义SVG中间稿和可编辑PPT，再使用桌面版PowerPoint的真实渲染结果进行迭代校正，目标像素还原度不低于90%。

> **Naming compatibility:** SlideTwin is the new public name and `$slidetwin` is the primary Skill invocation. The legacy plugin ID `ppt-visual-reconstructor`, marketplace ID `ppt-visual-tools`, and Skill invocation `$ppt-visual-reconstructor` remain supported so existing installations, prompts, and demo videos continue to work.

## Real image-only demo / 实测演示

The following public example was reconstructed from the reference PNG alone. The downloadable presentation is not a flattened slide: its single slide contains 358 PowerPoint objects, including 101 editable text objects, 223 shapes, 130 connectors, and 5 deliberately preserved raster images.

下面的公开案例仅使用左侧参考图片完成重建。可下载的PPT并非整页截图：单页包含358个PowerPoint对象，其中有101个可编辑文本对象、223个形状、130条连接线，以及5张有意保留的复杂图片。

<table>
  <tr>
    <th width="50%">Reference image / 原始参考图</th>
    <th width="50%">Editable PowerPoint render / 可编辑PPT渲染结果</th>
  </tr>
  <tr>
    <td><img src="docs/demo/reference.png" alt="Reference slide image"></td>
    <td><img src="docs/demo/reconstructed.png" alt="PowerPoint reconstruction rendered by desktop PowerPoint"></td>
  </tr>
</table>

| Measured result | Value |
| --- | ---: |
| Pixel similarity | `90.33%` |
| Composite visual score | `86.88%` |
| Edge similarity | `95.38%` |
| PowerPoint objects | `358` |
| Editable text objects | `101` |
| Preserved raster images | `5` |

[Download the editable demo PPTX](docs/demo/editable-demo.pptx) · [View the difference heatmap](docs/demo/difference-heatmap.png) · [Read the reproducible demo report](docs/demo/README.md)

<details>
  <summary><strong>Watch the 30-second workflow recording / 查看30秒真实操作录屏</strong></summary>
  <br>
  <img src="docs/demo/workflow.gif" alt="Real workflow recording" width="100%">
</details>

The numbers above belong to this second public test and were generated by the repository's own inspection and comparison scripts. They are separate from the first golden-sample metrics reported below.

## Status

Version `0.5.0` is the first SlideTwin public release. It adds:

- The distinct SlideTwin public brand and primary `$slidetwin` Skill invocation, with the previous identifiers retained as compatibility aliases.
- A unique legacy-compatible `ppt-visual-tools` Codex marketplace, avoiding collisions with a generic `personal` marketplace.
...
plugins/ppt-visual-reconstructor/  # retained internal compatibility ID
├── .codex-plugin/plugin.json
├── .mcp.json
├── scripts/
└── skills/
    ├── slidetwin/                 # primary Skill
    │   ├── SKILL.md
    │   └── agents/openai.yaml
    └── ppt-visual-reconstructor/  # compatibility alias and shared workflow
        ├── SKILL.md
        ├── agents/openai.yaml
        └── references/
```
...
The install identifiers above intentionally remain unchanged for backwards compatibility. The primary user-facing Skill is `$slidetwin`.

Start a new Codex thread after installation so the skill metadata is reloaded. Upload one slide image and use:

```text
Use $slidetwin to create an editable PowerPoint slide from only this image. Automatically keep photographs, artistic lettering, and complex illustrations as separate pictures; rebuild all text and simple geometry as native PowerPoint objects; and iterate toward at least 90% pixel similarity.
```

The earlier `$ppt-visual-reconstructor` prompt remains valid as a compatibility alias, including in existing workflow recordings.
